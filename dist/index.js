...
var importTypeSchema = zod.z.enum([
  "relative",
  "workspace",
  "alias",
  "external",
...
var importResolutionSchema = zod.z.object({
  ...
  resolvedPath: zod.z.string().optional(),
...
var importResolutionMetadataSchema = zod.z.record(
  zod.z.string(),
...
var relationshipFailureSchema = zod.z.object({
  /** Relative file path */
  file: zod.z.string(),
  /** Number of relationships that failed to create */
  failedCount: zod.z.number().int().nonnegative(),
  /** Number of relationships that were successfully created */
  createdCount: zod.z.number().int().nonnegative(),
  /** Whether failures were due to transient errors (retryable) */
  isTransient: zod.z.boolean()
});
var relationshipSummarySchema = zod.z.object({
  /** Total number of relationships created across all files */
  totalCreated: zod.z.number().int().nonnegative(),
  /** Total number of relationships that failed to create */
  totalFailed: zod.z.number().int().nonnegative(),
  /** Files that had at least one relationship failure */
  filesWithFailures: zod.z.array(relationshipFailureSchema)
});
var indexingResponseSchema = zod.z.object({
...
var updateErrorReportSchema = zod.z.object({
  status: zod.z.enum(["resolved", "unresolved"])
});
...
exports.serializedAstSchema = serializedAstSchema;
